...
- [[#wide-variety-of-inputs][Wide variety of inputs]]
  ...
  - [[#virtual-input-columns-with-precompute][Virtual input columns with :precompute]]
...
- [[#tricks][Tricks]]
  ...
  - [[#look-at-the-first-rows-of-a-large-table][Look at the first rows of a large table]]
...
** Virtual input columns with =:precompute=
:PROPERTIES:
:CUSTOM_ID: virtual-input-columns-with-precompute
:END:
...
Here =:precompute= added a third column names =$3=. It is computed with a
Lisp expression which:
- =(string-to-number $1)=: converts the first column to a Lisp number,
- =(/ … 100)=: divides it by 100,
- =(floor (/ …))=: rounds the result to an integer.

Note the single quote before the Lisp expression. This is the way to
specify a Lisp expression in Org-Mode spreadsheet.
...
The =:precompute= feature is by far the slowest in OrgAggregate. For
instance, a 3820 lines CSV file gets aggregated in less than 0.2
seconds. But with a =:precompute= expression adding a new column, time
goes up to 57 seconds (a ×285 slowdown).

This is because =:precompute= delegates the computing to Org-Mode
spreadsheet. Behind the scene,
- the input table is inserted as a temporary Org-Mode table,
- the =:precompute= formula is added,
- Org-Mode spreadsheet refreshes the table,
- then this temporary table becomes the new input before being erased.

OrgAggregate has been designed & optimized for speed. Thus it can
exhibit response times of fractions of second, even for tables with
thousands of rows.
...
** Look at the first rows of a large table
:PROPERTIES:
:CUSTOM_ID: look-at-the-first-rows-of-a-large-table
:END:

We have a table with thousands of rows. We are interested just in its
header, and maybe the first few rows. OrgAggregate can provide that:

#+begin_example
#+BEGIN: aggregate :table "largetable.csv:(csv header)[0:3]"
| day | color  | quantity |
|-----+--------+----------|
| thu | red    |      115 |
| fri | yellow |       78 |
...
The trick is to use slicing. Here we specified =[0:3]= to retrieve only
the 4 first rows of the huge input. We omitted the =:cols= parameters,
therefore asking OrgAggregate to provide all input columns.
