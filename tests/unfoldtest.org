...
(load-file "ensure-org-id-locations.el")
...
- Create a new aggregation (or copy-paste and adjust an old one).
- Duplicate it, without blank lines between the 2 copies.
- Put the cursor on the first copy, on the =#+begin:= line
- Record a macro with =C-x (=
- Type something to modify the aggregation
- End with cursor on the =#+begin:= line, and type =C-c C-c= to refresh the aggregation
- End recording the macro with =C-x )=
- Insert the recorded macro with =M-x insert-kbd-macro=
- Surround the recorded key strokes with a block like this one just above the freshly updated block.
